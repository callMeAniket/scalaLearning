object Basics {
  def main(args: Array[String]): Unit = {
    ...
    def calculateDonutCostWithOption(donutName: String, quantity: Int, couponCode: Option[String]): Unit = {
      couponCode match {
        case Some(coupon) =>
          println(coupon)

        case _ => println("Nothing is passed")
      }
    }

    calculateDonutCostWithOption("Aniket", 2, Some("optional"))
    calculateDonutCostWithOption("Aniket", 2, None)

    def calculateDonutCostWithDefaultOptionValue(donutName: String, quantity: Int, couponCode: Option[String] = None): Unit = {
      couponCode.map(x => println(x))
      couponCode match {
        case Some(coupon) =>
          println(coupon)

        case _ => println("Nothing is passed, but default value is " + couponCode)
      }
    }

    calculateDonutCostWithDefaultOptionValue("Aniket", 3, Some("Apply Code"))
    calculateDonutCostWithDefaultOptionValue("Aniket", 3)
  }
}